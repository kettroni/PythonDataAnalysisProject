...
class Regression(unittest.TestCase):
    ...
    def test_01_load(self):
        #exec(exercises[0])
        describe_method = spy_decorator(pd.core.frame.DataFrame.describe)
    ...
    def test_07_sbp_with_interactions_visualization(self):
        plot_method = spy_decorator(pd.core.frame.DataFrame.plot.scatter)
    ...
    def test_11_high_blood_pressure3(self):
        scatter_method = spy_decorator(matplotlib.axes.Axes.scatter)
        plot_method = spy_decorator(matplotlib.axes.Axes.plot)
#        with patch("matplotlib.pyplot.scatter", wraps=matplotlib.pyplot.scatter) as pltscatter:
        with patch("matplotlib.pyplot.subplots", wraps=matplotlib.pyplot.subplots) as psubplots,\
            ...
            self.assertIn(len(args1[1]), [663, 731], msg="Incorrect number of points in subfigure 2!")
    ...
    def test_13_cross_validation(self):
        ...
        self.assertAlmostEqual(np.mean(myerror_model), 0.3311827956989248, places=4, msg="Incorrect mean error rate!")
        self.assertAlmostEqual(np.mean(myerror_null), 0.3488530465949821, places=4, msg="Incorrect mean error rate by null model!")
